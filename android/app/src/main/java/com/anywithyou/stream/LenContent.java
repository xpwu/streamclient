...
class LenContent implements Net {
  ...
  static private class Connection {
    ...
    Connection(Config config, Delegate delegate) {
      this.config = config;
      this.onConnected = new DelegateHandler<Socket>() {
        @Override
        public void run(Socket _socket) {
          ...
          onConnected = new DelegateHandler<Socket>() {
            @Override
      ...
      this.onClosed = new DelegateHandler<String>() {
        @Override
        public void run(String d) {
          delegate.onClosed(d);
          onClosed = new DelegateHandler<String>() {
            @Override
            public void run(String d) {
            }
          };
      ...
      this.onError = new DelegateHandler<Error>() {
        @Override
        public void run(Error d) {
          delegate.onError(d);
          onError = new DelegateHandler<Error>() {
            @Override
            public void run(Error d) {
            }
          };
      ...
      this.onMessage = new DelegateHandler<byte[]>() {
        @Override
...
//      timer.cancel();
    ...
    public void invalidAndClose() {
      // 失效后，不再向delegate发消息
      onMessage = new DelegateHandler<byte[]>() {
        @Override
        public void run(byte[] d) {
        ...
        public void run() {
          ...
          while (true) {
            ...
            try {
              ...
              if (n <= 0) {
                handleError(handler, new Error("inputstream read error, maybe connection closed by peer"));
                break;
              }
              pos += n;
